...
def keyPressP1UP(event):
    if GPIO.input(INPUT_CHANNEL[0]):
        keydown(P1UP)
    else:
        keyup(P1UP)

def keyPressP1DOWN(event):
    if GPIO.input(INPUT_CHANNEL[1]):
        keydown(P1DOWN)
    else:
        keyup(P1DOWN)

def keyPressP2UP(event):
    if GPIO.input(INPUT_CHANNEL[2]):
        keydown(P2UP)
    else:
        keyup(P2UP)

def keyPressP2DOWN(event):
...
#        GPIO.add_event_detect(INPUT_CHANNEL[i], GPIO.RISING)  # add rising edge detection on a channel
        GPIO.add_event_detect(INPUT_CHANNEL[i], GPIO.BOTH)  # add rising edge detection on a channel

GPIO.add_event_callback(INPUT_CHANNEL[0], keyPressP1UP)
GPIO.add_event_callback(INPUT_CHANNEL[1], keyPressP1DOWN)
GPIO.add_event_callback(INPUT_CHANNEL[2], keyPressP2UP)
GPIO.add_event_callback(INPUT_CHANNEL[3], keyPressP2DOWN)
...
pi@raspberrypi ~/pianopong $ ^C
pi@raspberrypi ~/pianopong $ cat pongv1.gpio.py
import threading
import time
